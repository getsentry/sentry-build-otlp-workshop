...
1. **Service** sends telemetry with `service.name` attribute
2. **Collector** receives telemetry at OTLP receiver
3. **Routing connector** evaluates `service.name`:
   - If `products-service` → routes to `traces/products` pipeline
   - If `orders-service` → routes to `traces/orders` pipeline
4. **Pipeline exporters** send to corresponding Sentry projects

## Benefits

- **Isolation**: Each service's data in separate Sentry projects
- **Team Ownership**: Different teams can manage their projects
- **Flexibility**: Different sampling/retention rules per project
- **Cost Control**: Track and manage costs per service

## When to Use

**Use multi-project routing when:**

- You have distinct services owned by different teams
- You need different retention/sampling per service
- You want to track costs separately
- You need project-level access control

**Use single project when:**

- All services are managed by one team
- You want unified views across all services
